...
class ClassBalancedCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, samples_per_cls, beta=0.999, reduction="mean"):
        """
        samples_per_cls : list[int]  # number of original samples from each class
        beta            : float      # recommended between 0.9 and 0.999 (if 0, same as standard CE)
        """
        super().__init__()
        eff_num = 1.0 - np.power(beta, samples_per_cls)
        weights = (1.0 - beta) / np.array(eff_num)
        weights = weights / weights.sum() * len(samples_per_cls)          # normalization
...
class ClassBalancedFocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        if self.reduction == "mean":
            return loss.sum() / (cb_w.sum() + 1e-12)  # <- same scale as CE
